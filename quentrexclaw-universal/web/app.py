...
if st.button("ROCK THE PREP"):
    if not in_kz:
        ...
        st.success("✅ Data Pack loaded (Orion + Whales + Liq Heatmap)")
        st.subheader("6-Line Analysis")
        st.markdown("**1. Confluence: 94/100** – VWAP reclaim + Orion spike + whale flow\n**2. SMB: 4.9/5** – Strong SMT\n**3. Trigger: Sweep→Reclaim VAL**\n**4. Decision: GO**\n**5. Plan: Long @148.80 | SL 148.10 (0.47%) | TP1 150.50 | TP2 152.20 | Exit 22:05 MMT**\n**6. Notes: No news | Bias flipper only if funding flips**")
        if st.button("CONFIRM + EXECUTE REAL"):
            st.balloons()
            st.success("TINY TRADE EXECUTED on MEXC (0.42% risk) – Monitoring...")
